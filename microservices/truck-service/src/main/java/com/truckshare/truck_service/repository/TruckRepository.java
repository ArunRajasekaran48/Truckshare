...
public interface TruckRepository extends JpaRepository<Truck, UUID> {
   @Query("SELECT t FROM Truck t WHERE t.status = :status AND t.fromLocation = :from AND t.toLocation = :to AND t.availableWeight >= :requiredWeight AND t.availableVolume >= :requiredVolume AND t.availableLength >= :requiredLength")
    List<Truck> findByFromLocationAndToLocationWithCapacity(@Param("from") String from,
                                                       ...
                                                       @Param("requiredLength") Double requiredLength,
    ...
    List<Truck> findByDriverId(String driverId);
}
